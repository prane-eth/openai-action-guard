...
Client-side action guards are available on `client.chat.completions.create()`, `client.chat.completions.parse()`, `client.responses.create()`, and `client.responses.parse()`. They are SDK-only arguments, blocked actions raise `openai.ActionGuardError`.
